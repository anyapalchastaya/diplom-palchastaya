...
import redis
from boards.models import Notification, Board
...
class ProjectBoards(BoardList):

    def get_queryset(self, *args, **kwargs):
        pk = self.kwargs['pk']

        search = self.request.GET.get('q', None)
        queryset = Board.objects.filter(project_id=pk)

        if search is not None:
            queryset = queryset.filter(title__icontains=search)[:2]

        return queryset

class ConnectBoardToProject(generics.GenericAPIView):
...
class DisconnectBoardFromProject(generics.GenericAPIView):
    permission_classes = [IsProjectMember]
