...
from .models import Attachment, Board, Comment, Item, Label, List, Notification, CardMovement
...
class ItemList(generics.ListCreateAPIView):
    ...
    def create_card_movement(self, item, list):
...
class ItemDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if "due_date" in request.data:
            request.data["due_date"] = datetime.strptime(request.data["due_date"], "%Y-%m-%d %H:%M")
        ...
        if "labels" in request.data:
            ...
            if label is None:
                return Response({"labels": ["This label does not belong to this board"]}, status=status.HTTP_400_BAD_REQUEST)

        if "list" in request.data:
            list = self.get_list(request.data['list'], item.list.board)
            if list is None:
                return Response({'list': ["This list doesn't belong to this board"]}, status=status.HTTP_400_BAD_REQUEST)
...
class CardMovementStats(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        period = request.GET.get('period', None)
        board_id = request.GET.get('board_id', None)

        if period is None:
            return Response({"error": "Period not provided."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        card_movements = CardMovement.objects.filter(timestamp__range=(start_date, end_date),
                                                     moved_by=request.user,
                                                     item__list__board_id=board_id).order_by('timestamp')

        stats = defaultdict(lambda: defaultdict(int))

        for movement in card_movements:
            date_str = movement.timestamp.strftime('%Y-%m-%d')
            to_list = movement.to_list.title

            stats[date_str][to_list] += 1

        formatted_stats = []

        for date_str, lists in stats.items():
            daily_stats = {"date": date_str}
            daily_stats.update(lists)
            formatted_stats.append(daily_stats)
